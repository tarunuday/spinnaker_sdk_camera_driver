...
//	 ROS_DEBUG_STREAM("  Write Queue to Disk for cam: "<< cam_no <<" size = "<<img_q->size());
	 ...
	 uint64_t timeStamp =  convertedImage->GetTimeStamp() * 1000;
	  ...
	  ostringstream filename;
	  filename<<path_<<cam_names_[cam_no]<<"/"
	           <<cam_names_[cam_no]<<"_"<<id<<"_"<<todays_date_ << "_"<<std::setfill('0') << std::setw(6) << imageCnt<<"_"<<timeStamp << ext_;; 
			//<<ext_;
		...
		for (int imageCnt = 0; imageCnt < k_numImages; imageCnt++)
		{
			...
			for (int i = 0; i < numCameras_; i++)
			{
				try
				{
			           ...
			           filename << cam_names_[i]<<"_"<<cam_ids_[i].c_str()<< "_"<<todays_date_ << "_"<< std::setfill('0') << std::setw(6) << imageCnt<<"_"<<timeStamp << ext_;
			           imageNames.push_back(filename.str());
						...
						//~ {
							...
							//~ if(i == 0){
								//~ cam_msg1 = cv_bridge::CvImage(std_msgs::Header(), "mono8", img).toImageMsg();
								//~ calib_pub1 .publish(cam_msg1);
							//~ }
							//~ else if(i == 1){
								//~ cam_msg2 = cv_bridge::CvImage(std_msgs::Header(), "mono8", img).toImageMsg();
								//~ calib_pub2 .publish(cam_msg2);
							//~ }
							//~ else if(i == 1){
								//~ cam_msg3 = cv_bridge::CvImage(std_msgs::Header(), "mono8", img).toImageMsg();
								//~ calib_pub3 .publish(cam_msg3);
							//~ }
							//~ else if(i == 1){
								//~ cam_msg4 = cv_bridge::CvImage(std_msgs::Header(), "mono8", img).toImageMsg();
								//~ calib_pub4 .publish(cam_msg4);
							//~ }
							//~ else
							//~ {
								//~ cam_msg5 = cv_bridge::CvImage(std_msgs::Header(), "mono8", img).toImageMsg();
								//~ calib_pub5 .publish(cam_msg5);
							//~ }
